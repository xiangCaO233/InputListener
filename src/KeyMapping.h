...
/**
 * @brief macOS Shift 状态下的虚拟键码到文本映射表。
 */
...
#ifdef __unix__
/**
 * @brief Linux 普通状态下的输入子系统键码到文本映射表。
 */
...
/**
 * @brief Linux Shift 状态下的输入子系统键码到文本映射表。
 */
...
/**
 * @brief 根据平台键码和 Shift 状态获取可显示按键文本。
 *
 * @param code 平台原始键码。
 * @param shiftPressed Shift 键是否处于按下状态。
 * @return 映射到的按键文本；没有映射时返回空字符串。
 */
inline const char *keyTextForCode(int code, bool shiftPressed) {
#ifdef __APPLE__
  const auto &keyMap = shiftPressed ? shiftKeyMap : normalKeyMap;
#elif defined(__unix__)
  const auto &keyMap = shiftPressed ? linux_shift_keymap : linux_keymap;
#else
