...
std::condition_variable hooksReadyCondition; ///< 通知 Hook 启动结果的条件变量。
...
Modifiers snapshotModifiers() {
  Modifiers modifiers;
  modifiers.shift = isKeyDown(VK_SHIFT) || isKeyDown(VK_LSHIFT) ||
                    isKeyDown(VK_RSHIFT);
...
std::string keyTextForWindowsKey(UINT virtualKey, UINT scanCode,
  ...
  wchar_t buffer[8] = {};
  int count = ToUnicodeEx(virtualKey, scanCode, keyboardState, buffer,
...
void dispatchKeyEvent(UINT virtualKey, UINT scanCode, KeyEventType type) {
  ...
  if (type == KeyEventType::RELEASE) {
    KeyEvent pressedEvent(static_cast<int>(virtualKey), key, modifiers,
...
void dispatchMouseEvent(int button, MouseEventType type, const POINT &point,
  ...
  Modifiers modifiers = snapshotModifiers();
  MouseEvent event(button, modifiers, type, static_cast<int>(point.x),
                   static_cast<int>(point.y), static_cast<double>(point.x),
                   static_cast<double>(point.y), scrollDeltaX, scrollDeltaY);
...
void dispatchMouseButtonEvent(int button, bool pressed, const POINT &point) {
  ...
  dispatchMouseEvent(button,
                     dragged ? MouseEventType::DRAGGED
...
HMODULE moduleForHookProcs() {
  ...
  GetModuleHandleExW(
      ...
      reinterpret_cast<LPCWSTR>(&keyboardHookProc), &module);
  return module;
}
...
void hookLoop() {
  ...
  keyboardHook = SetWindowsHookExW(WH_KEYBOARD_LL, keyboardHookProc, module, 0);
...
LRESULT CALLBACK keyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (nCode == HC_ACTION && listening.load()) {
    ...
    if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
      ...
      dispatchKeyEvent(virtualKey, scanCode,
...
LRESULT CALLBACK mouseHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (nCode == HC_ACTION && listening.load()) {
    ...
    switch (wParam) {
    case WM_MOUSEMOVE:
      dispatchMouseMoveEvent(info->pt);
      break;
    case WM_LBUTTONDOWN:
    ...
    case WM_MBUTTONUP:
      ...
      break;
    case WM_XBUTTONDOWN:
      dispatchMouseButtonEvent(xButtonForMouseData(info->mouseData), true,
                               info->pt);
      break;
    case WM_XBUTTONUP:
      dispatchMouseButtonEvent(xButtonForMouseData(info->mouseData), false,
                               info->pt);
      break;
    case WM_MOUSEWHEEL: {
      double delta = static_cast<SHORT>(HIWORD(info->mouseData)) /
                     static_cast<double>(WHEEL_DELTA);
      dispatchMouseEvent(buttonNone, MouseEventType::SCROLL, info->pt, 0.0,
                         delta);
      break;
    }
    case WM_MOUSEHWHEEL: {
      double delta = static_cast<SHORT>(HIWORD(info->mouseData)) /
                     static_cast<double>(WHEEL_DELTA);
      dispatchMouseEvent(buttonNone, MouseEventType::SCROLL, info->pt, delta,
                         0.0);
      break;
    }
    default:
