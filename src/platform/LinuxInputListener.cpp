 ...
 */
bool isKeyboardKey(unsigned short code) { return code < BTN_MISC; }
...
void updateModifiers(unsigned short code, int value) {
  ...
  case KEY_LEFTSHIFT:
  case KEY_RIGHTSHIFT:
    globalModifiers.shift = pressed;
    break;
  case KEY_LEFTCTRL:
  case KEY_RIGHTCTRL:
    globalModifiers.control = pressed;
    break;
  case KEY_LEFTALT:
  case KEY_RIGHTALT:
    globalModifiers.option = pressed;
    break;
  case KEY_LEFTMETA:
  case KEY_RIGHTMETA:
    globalModifiers.command = pressed;
    break;
  default:
    break;
  }
...
DeviceInfo inspectDevice(const std::string &devicePath) {
  ...
  bool hasEvents =
      ioctl(fd, EVIOCGBIT(0, sizeof(evbit)), evbit.data()) >= 0;
  bool hasKeys =
...
void dispatchKeyEvent(InputEventSink &sink, unsigned short code, int value) {
  ...
  KeyEvent event(code, keyTextForCode(code, modifiers.shift), modifiers, type);
...
void dispatchMouseButtonEvent(InputEventSink &sink, MouseState &state,
  ...
  if (value != 0) {
    ...
    state.dragged = false;
    MouseEvent event(button, modifiers, MouseEventType::PRESS, state.x,
                     state.y, state.x, state.y);
    sink.dispatchMouseEvent(event);
    return;
  }

  MouseEvent releaseEvent(button, modifiers, MouseEventType::RELEASE, state.x,
                          state.y, state.x, state.y);
  ...
  MouseEvent completeEvent(button, modifiers, completeType, state.x, state.y,
                           state.x, state.y);
    ...
    if (n == static_cast<ssize_t>(sizeof(ev))) {
      if (ev.type == EV_KEY && (ev.value == 0 || ev.value == 1 ||
        ...
        if (device.keyboard && isKeyboardKey(ev.code)) {
          ...
          dispatchMouseButtonEvent(sink, mouseState, ev.code, ev.value);
        }
