                         ...
                         CGEventRef event, void* refcon)
{
    ...
    const char*  key   = keyTextForCode(keyCode, flags & kCGEventFlagMaskShift);
    ...
    if ( type == kCGEventMouseMoved ) {
        sendMouseEvent(
            *sink, 0, MouseEventType::MOVE, modifiers, mouseLocation);
    } else if ( type == kCGEventLeftMouseDragged ) {
        sendMouseEvent(
            *sink, 1, MouseEventType::DRAG, modifiers, mouseLocation);
    } else if ( type == kCGEventRightMouseDragged ) {
        sendMouseEvent(
            *sink, 2, MouseEventType::DRAG, modifiers, mouseLocation);
    } else if ( type == kCGEventOtherMouseDragged ) {
        sendMouseEvent(
            *sink, 3, MouseEventType::DRAG, modifiers, mouseLocation);
        ...
        sendMouseEvent(
            *sink, 1, MouseEventType::PRESS, modifiers, mouseLocation);
    } else if ( type == kCGEventLeftMouseUp ) {
        sendMouseEvent(
            *sink, 1, MouseEventType::RELEASE, modifiers, mouseLocation);
        sendMouseEvent(
            *sink, 1, MouseEventType::PRESSED, modifiers, mouseLocation);
    } else if ( type == kCGEventRightMouseDown ) {
        sendMouseEvent(
            *sink, 2, MouseEventType::PRESS, modifiers, mouseLocation);
    } else if ( type == kCGEventRightMouseUp ) {
        sendMouseEvent(
            *sink, 2, MouseEventType::RELEASE, modifiers, mouseLocation);
        sendMouseEvent(
            *sink, 2, MouseEventType::PRESSED, modifiers, mouseLocation);
    } else if ( type == kCGEventOtherMouseDown ) {
        sendMouseEvent(
            *sink, 3, MouseEventType::PRESS, modifiers, mouseLocation);
    } else if ( type == kCGEventOtherMouseUp ) {
        sendMouseEvent(
            *sink, 3, MouseEventType::RELEASE, modifiers, mouseLocation);
        sendMouseEvent(
            *sink, 3, MouseEventType::PRESSED, modifiers, mouseLocation);
    }
